...
import 'friend_message_card.dart';
...
class _ChatState extends State<Chat> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      _token = Provider.of<LoginProvider>(context, listen: false).tokenuser;
      idUser = Provider.of<LoginProvider>(context, listen: false).userid;
      if (mounted) {
        Provider.of<ConversationProvider>(context, listen: false)
            .getConversation(_token);
        _timer =
            Timer.periodic(const Duration(seconds: 1), realTimeConversations);
  ...
  realTimeConversations(Timer t) async {
    try {
      List<ConversationModel> conv =
          Provider.of<ConversationProvider>(context, listen: false)
              .conversations;
      var conversationsUpdated =
          conv.firstWhere((element) => element.id == widget.conversation.id);
      setState(() {
        ...
        if (_scrollController.position.pixels ==
          ...
          if (_conversationsUpdated.last.read.toString() == '0') {
            if (_conversationsUpdated.last.userId != idUser) {
              Provider.of<ConversationProvider>(context, listen: false)
                  .makeConversationAsReaded(
                      _conversationsUpdated.last.conversationId, _token);
            }
          }
        }
      }
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return _conversation[index].userId != idUser
                          ? FriendMessageCard(message: _conversation[index])
                          : MyMessageCard(message: _conversation[index]);
                    }),
          ),
        ),
        _conversation.isEmpty
            ? Container()
            : _conversation.last.read == '1'
                ? Container()
                : _conversation.last.userId == idUser
                    ? Container()
                            ...
                            : InkWell(
                                onTap: () async {
                                  ...
                                  await Provider.of<ConversationProvider>(
                                          ...
                                          listen: false)
                                      .makeConversationAsReaded(
                                          _conversation.last.conversationId,
                                          _token);
                                },
                                child: Container(
                                  width: 250,
                  ...
                  child: InkWell(
                    onTap: () async {
                      ...
                      await Provider.of<ConversationProvider>(context,
                              listen: false)
                          .sendMessages(message, _token);
                      if (_scrollController.positions.isNotEmpty) {
                        _scrollController.jumpTo(
                            _scrollController.position.maxScrollExtent + 15);
                      }
